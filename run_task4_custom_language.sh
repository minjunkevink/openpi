...
echo "  --num-rollouts-per-prompt N    (default: 1)"
echo "  --use-wandb false              (to disable wandb)"
echo "  --wandb-project PROJECT_NAME  (default: openpi-libero-task4-custom-language)"
echo "  --state-noise-std N             (default: 0.0, add noise to state for robustness testing)"
echo "  --state-noise-type TYPE        (default: gaussian, options: gaussian, uniform, mask)"
echo "  --language-type TYPE           (default: custom, for wandb naming: goal_state, paraphrase, etc.)"
echo "=========================================="
echo ""
